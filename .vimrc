...
" Ctrl + C                   --单源文件编译           [已配置 C Coffee]
" Ctrl + R                   --单源文件运行           [已配置 C PHP Rb Coffee Bash]
" Ctrl + ]                   --转到函数定义
" Ctrl + T                   --返回调用函数
...
" jj                         --保存文件并留在插入模式 [插入模式] [暂时已注释]
" kk                         --返回Normal模式         [插入模式] [暂时已注释]
" nt                         --打开NERDTree           [非插入模式]
" tl                         --打开TagList            [非插入模式]
...
" 每行超过80个的字符用下划线标示
au BufRead,BufNewFile *.c,*.h,*.php,*.rb,*.coffee,*.sh,*.sql,*.vim,*.js,*.css,*.html 2match Underlined /.\%81v/
...
\ 'auto_export': 1,
\ 'nested_syntaxes': {'Clang': 'c', 'PHP': 'php', 'Ruby': 'ruby', 'Coffee': 'coffee', 'Bash': 'sh', 'SQL': 'sql', 'Vim': 'vim', 'JS': 'javascript', 'CSS': 'css', 'HTML': 'html', 'XML': 'xml'},}]
